...
export function CartItem({
  ...
  const stepperButton =
    'flex h-10 w-10 items-center justify-center text-slate-600 transition-colors hover:bg-brand-50 hover:text-brand-700 disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent';

  return (
    <li
      className={classNames(
        'group flex flex-col gap-4 py-5 transition-opacity duration-200 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:gap-5',
        busy && 'opacity-60'
      )}
    >
      <div className="flex min-w-0 flex-1 items-center gap-4">
        <Link
          ...
          className="h-20 w-20 shrink-0 overflow-hidden rounded-card bg-slate-100 ring-1 ring-slate-200/70 transition-[box-shadow,transform] duration-300 ease-entrance hover:-translate-y-0.5 hover:shadow-card-hover sm:h-24 sm:w-24"
          tabIndex={-1}
          aria-hidden="true"
        >
          {product.image ? (
            <img
              src={product.image}
              alt=""
              className="h-full w-full object-cover transition-transform duration-500 ease-entrance group-hover:scale-105"
            />
          ) : (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 sm:justify-end sm:gap-5">
        <div className="flex h-10 items-center overflow-hidden rounded-control border border-slate-300 bg-white shadow-xs">
          <button
            type="button"
          ...
          <span
            className="w-9 border-x border-slate-200 text-center text-sm font-bold tabular-nums text-slate-900"
            aria-live="polite"
          >
            {quantity}
          </span>
        ...
        <span className="w-24 shrink-0 text-right text-base font-extrabold tabular-nums tracking-tight text-slate-900">
